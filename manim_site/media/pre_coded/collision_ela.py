...
parser.add_argument("--output_name", type=str, required=True, help="Nom du fichier de sortie")
...
if not (-15 < args.v2 < args.v1 <= 15):
    sys.exit("Erreur : La vitesse v2 doit être inférieure à v1, supérieure à -15 m/s, et v1 ne doit pas dépasser 15 m/s.")
...
# Calcul des vitesses après la collision élastique
...
fig, ax = plt.subplots(figsize=(10, 4))  # Ajustement de la taille de l'image, largeur=10, hauteur=3.5
ax.set_xlim(-5, 25)
...
# Sauvegarder l'animation avec le nom de fichier personnalisé
ani.save(os.path.join(output_directory, f"output_{args.output_name}.mp4"), writer='ffmpeg', fps=30)
